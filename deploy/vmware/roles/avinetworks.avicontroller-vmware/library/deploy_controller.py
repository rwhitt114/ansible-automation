...
def main():
    module = AnsibleModule(
	    ...
	    #con_cluster_folder=dict(required=False, type='str', default=False),
    ...
    #if con_cluster_folder:
    #    vi_string += '/%s/host/%s/%s' % (dc.name, con_cluster_folder, cl.name)
    #else:
    vi_string += '/%s/host/CHA/%s' % (dc.name, cl.name)
